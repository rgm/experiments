(ns user
  "Try using redis streams as the ledger/backbone for a commander architecture.
   ...
   DRAMATIS PERSONAE:
   evt ......... Event, a map
   cmd ......... Command, a map, found in pairs of Events
...
(def schema:evt
  "Events are records of effects that have happened in the system.

   Events are named with lower-case past-tense voice symbols, for example
   `'a-thing-happened`.

   Events can be observed for secondary reactions (eg. updating the UI) or
   updating derived data (eg. the audit log on a project).
...
(def schema:cmd
  "Commands are expressions of a user's intent, as data.
   ...
   Commands are named with all-caps imperative voice symbols, for example
   `'DO-A-THING!'

   They are observable in the event ledger as (issued, accepted) or
   (issued, rejected) pairs of events. (In some parts of a system, we might see
   a transient (issued, bounced) pair, signifying something didn't even make
   it into the ledger, and that's usually for obvious authz reasons like a
   read-only user).

   They flow into the ledger, where the fact of a command being issued is
   immediately persisted as an event. A command/issued event wrapping the
   command flows into the commander, where it's acted on, and will lead to
   either an accepted event or a rejected event (plus some optional other side
   events that happened. Note that the commander issuing a command in response
   is also a kind of side event :mindblown:)."
  [:map {:closed true}
   [:cmd/id   uuid?]
...
(def schema:issued-cmd
  "Regardless of whether a command is ultimately accepted or rejected,
   ...
   always true ... a command was issued)."
  (mu/merge schema:evt
            [:map
...
(defmethod ig/init-key :user/state
  ...
  (atom
   {:counter counter
    ...
    ;; starts up).
    :last-eids {:user/commander 0 :user/archiver 0}}))
